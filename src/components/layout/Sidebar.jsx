...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`${isOpen ? "w-64" : "w-20"} bg-cce-dark text-white h-screen flex flex-col transition-all duration-300`}
    >
      {/* --- Top Section / Logo --- */}
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 w-full px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150
                ${active ? "bg-cce-light text-cce-dark" : "hover:bg-cce-light/30 hover:text-white"}`}
            >
              {item.icon}
